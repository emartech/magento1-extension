...
create-test-db: ## Creates magento-test database
	@$(COMPOSE) exec db bash -c 'mysqldump -u root -p${MYSQL_ROOT_PASSWORD} magento1_test > /opt/magento_test.sql'

reset-test-db:
	@$(COMPOSE) exec db bash -c 'mysql -u root -p${MYSQL_ROOT_PASSWORD} magento1_test < /opt/magento_test.sql'
