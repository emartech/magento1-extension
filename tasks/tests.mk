...
create-test-db: ## Creates magento-test database
	@$(COMPOSE) exec --user root db bash -c 'mysqldump -u root -p${MYSQL_ROOT_PASSWORD} magento1_test > /opt/magento_test.sql'

reset-test-db:
	@$(COMPOSE) exec --user root db bash -c 'mysql -u root -p${MYSQL_ROOT_PASSWORD} magento1_test < /opt/magento_test.sql'
